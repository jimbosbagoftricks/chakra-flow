...
function _droneEnsure() {
  ...
  marker.gain.value = _audioMarkerGain();
...
function _audioMasterGain() {
  return _audioLoud ? CHANT_AUDIO.loudMasterGain : CHANT_AUDIO.masterGain;
}
function _audioMarkerGain() {
  return _audioLoud ? CHANT_AUDIO.loudMarkerGain : CHANT_AUDIO.markerGain;
}
function _audioSetLoud(enabled) {
  _audioLoud = !!enabled;
  if (!_drone) return;
  if (_drone.marker) _drone.marker.gain.value = _audioMarkerGain();
  if (!_drone.voices.length) return;
  const now = _drone.ctx.currentTime;
  _drone.master.gain.cancelScheduledValues(now);
  _drone.master.gain.setValueAtTime(Math.max(_drone.master.gain.value, 0.0001), now);
  _drone.master.gain.exponentialRampToValueAtTime(_audioMasterGain(), now + 0.35);
}
...
function _droneSet(hz) {
  ...
  a.master.gain.exponentialRampToValueAtTime(_audioMasterGain(), t0 + 0.9);
}
...
const CHANT_DEFAULTS = { inhale: 4, exhale: 4, pause: 1, reps: 9, advance: false, autoplay: false, loud: false };
...
function _chantMarker(baseHz, kind) {
  ...
  const a = _drone;
  if (a.ctx.state === "suspended") a.ctx.resume();
  a.marker.gain.value = _audioMarkerGain();
    ...
    <div role="dialog" aria-modal="true" aria-label="About Chakra Flow" onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        <AboutSection title="How To Use The App">
          ...
          <p>Tap <b style={{ fontWeight: 600 }}>♪</b> for the current card drone. Use <b style={{ fontWeight: 600 }}>Settings</b> to adjust tone tuning, bīja repetitions, chant pacing, pause length, countdown advancement, and loud speaker mode.</p>
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Options" onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 460, margin: "0 auto" }}>
        ...
        {toneControl}
        {toggleControl("loud", "Loud speaker mode", "Boosts the drone and chant markers for phone speakers. Leave this off with headphones.")}
...
export default function ChakraCards({ artBase = DEFAULT_ART, mantraArtBase = DEFAULT_MANTRA_ART }) {
  ...
  useEffect(() => {
    _audioSetLoud(chantSettings.loud);
  }, [chantSettings.loud]);
  useEffect(() => {
  ...
  const updateChantSetting = useCallback((key, value) => {
    setChantSettings((prev) => {
      if (key === "advance" || key === "autoplay" || key === "loud") {
  ...
  useEffect(() => {
    const onVisibility = () => {
      if (document.visibilityState === "hidden") stopPracticeAudio();
    };
    document.addEventListener("visibilitychange", onVisibility);
    window.addEventListener("pagehide", stopPracticeAudio);
    return () => {
      document.removeEventListener("visibilitychange", onVisibility);
      window.removeEventListener("pagehide", stopPracticeAudio);
    };
  }, [stopPracticeAudio]);
  ...
  const modalOpen = showOptions || showInfo || photoCard || breathHelpCard;
  const onStart = (e) => {
    if (modalOpen) { touch.current = null; return; }
    touch.current = e.touches[0].clientX;
  };
  const onEnd = (e) => {
    if (view !== "flow" || modalOpen) { touch.current = null; return; }
